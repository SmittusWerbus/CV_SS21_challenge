...
%                     end
                    ...
                    if(length(find(inlierIdx==1))<4)
                        ...
                        while(length(find(inlierIdx==1))<4)
                            ...
                            [tform{1,i},inlierIdx] = estimateGeometricTransform2D(copy_matchedPts{1,2},copy_matchedPts{1,1},'projective','MaxDistance',maxD);
                            copy_matchedPts{1,1} = copy_matchedPts{1,1}(inlierIdx,:);
                            copy_matchedPts{1,2} = copy_matchedPts{1,2}(inlierIdx,:);
%                             if(i~=length(obj.pic))
%                             figure;showMatchedFeatures(obj.pic{1,i-j}.picture,obj.pic{1,i+1}.picture,copy_matchedPts{1,1},copy_matchedPts{1,2},'montage');
%                             else
%                                figure;showMatchedFeatures(obj.pic{1,i-j}.picture,obj.pic{1,1}.picture,copy_matchedPts{1,1},copy_matchedPts{1,2},'montage'); 
%                             end
                            j=j+1;
                            if(j==i)
                                j=0;
...
%                     tform{1,i}.T=tform{1,good_picture}.T*tform{1,good_picture-1}.T;
...
%                     end
                    ...
                    if(i~=length(obj.pic))
                        for k=1:i
                            ...
                            elseif(k==1)
                                obj.pic{1,i+1}.SURF.rotated_picture = imwarp(obj.pic{1,i+1}.picture,tform{1,i-k+1});
                            elseif(k==i)
%                                 XLimit=tform{1,i-k+2}.T(3,:)*tform{1,i-k+1}.T(:,1);
%                                 YLimit=tform{1,i-k+2}.T(3,:)*tform{1,i-k+1}.T(:,2);
                                outputView = imref2d(size(obj.pic{1,i+1}.SURF.rotated_picture));
%                                 xlim=[XLimit,XLimit+outputView.ImageSize(1,2)];
%                                 ylim=[YLimit,YLimit+outputView.ImageSize(1,1)];
%                                 outputView.XWorldLimits=xlim;
%                                 outputView.YWorldLimits=ylim;
